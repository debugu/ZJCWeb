...
{
    /// <summary>
    /// ImportStudentInfo 的摘要说明
    /// </summary>
    public class ImportStudentInfo : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            List<StudentInfoModel> smodel = new List<StudentInfoModel>();
            using (FileStream file = new FileStream(fullDir, FileMode.Open, FileAccess.Read))
            {
                ...
                for (int i = (sheet.FirstRowNum + 1); i < sheet.LastRowNum + 1; i++)
                {
                    ...
                    s.HaveBY = false;
                    s.Sex = GetValue(row.GetCell(row.FirstCellNum + 2));
                    s.IDCardNO = GetValue(row.GetCell(row.FirstCellNum + 3));
                    s.HomeAddress = GetValue(row.GetCell(row.FirstCellNum + 5));
                    s.PassWord= GetValue(row.GetCell(row.FirstCellNum + 4));
                    s.IsDeLess = false;
                    s.IsHeartLess = false;
                    s.IsMigrants = false;
                    s.IsPoor = false;
                    s.IsStayAtHomeChild = false;
                    new StudentInfoBLL().AddNew(s);
                    smodel.Add(s);
                    StuClassModel sc = new StuClassModel();
                    sc.id = Guid.NewGuid();
                    sc.StuId = s.id;
                    sc.ClassId=new Guid(row.GetCell(row.FirstCellNum + 6).ToString());
                    new StuClassBLL().AddNew(sc);
                }
            }
